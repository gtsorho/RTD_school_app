...
export default {
  async create(req: Request, res: Response): Promise<any> {
    try {
      ...
      const existingTeacher = await db.teacher.findOne({ where: { user_id: value.user_id } });
      if (existingTeacher) return res.status(400).json({ error: "Teacher with this user_id already exists" });
